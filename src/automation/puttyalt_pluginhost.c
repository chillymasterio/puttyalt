...
int pluginhost_has_capability(const PluginHost *h, int idx, int cap) {
    if(!h||idx<0||idx>=h->n) return 0;
    return (h->plugins[idx].capabilities&cap)?1:0;
}
...
int pluginhost_active_count(const PluginHost *h) {
    if(!h) return -1;
    int n=0; for(int i=0;i<h->n;i++) if(h->plugins[i].state==PH_ACTIVE)n++; return n;
}
