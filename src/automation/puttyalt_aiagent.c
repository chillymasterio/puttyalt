...
void aiagent_init(AiAgent *a, const char *goal, int max_steps) {
    if(!a) return; memset(a,0,sizeof(*a)); snprintf(a->goal,AG_TEXT,"%s",goal?goal:"");
...
int aiagent_success_rate(const AiAgent *a) {
    if(!a||a->n==0) return 0; int ok=0; for(int i=0;i<a->n;i++) if(a->steps[i].success)ok++; return (ok*100)/a->n;
}
