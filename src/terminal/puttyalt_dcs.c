...
int dcs_start(Dcs *d, const char *params, char final) {
    if(!d) return -1; d->state=DCS_PASSTHROUGH; snprintf(d->params,64,"%s",params?params:""); d->final=final; d->data_len=0;
    return 0;
}
...
int dcs_terminate(Dcs *d) {
    if(!d||d->state!=DCS_PASSTHROUGH) return -1; d->state=DCS_IDLE; return d->data_len;
}
