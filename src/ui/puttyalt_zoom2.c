...
void zoom2_init(Zoom2 *z, int base_font, int dpi_scale) {
    if(!z) return;
    memset(z,0,sizeof(*z)); z->base_font=base_font>0?base_font:14; z->current_font=z->base_font;
...
int zoom2_effective_size(const Zoom2 *z) {
    if(!z) return 0;
    return z->current_font * z->dpi_scale / 100;
}
int zoom2_percent(const Zoom2 *z) {
    if(!z||z->base_font==0) return 100;
    return z->current_font*100/z->base_font;
}
