...
void paneborder_init(PaneBorder *p) {
    if(!p) return;
    memset(p,0,sizeof(*p)); p->active_pane=-1; p->border_width=1;
...
unsigned int paneborder_color(const PaneBorder *p, int pane) {
    if(!p) return 0;
    return (pane==p->active_pane)?p->active_color:p->inactive_color;
}
int paneborder_width(const PaneBorder *p, int pane) {
    if(!p) return 1;
    return (pane==p->active_pane)?p->border_width+1:p->border_width;
}
void paneborder_set_colors(PaneBorder *p, unsigned int active, unsigned int inactive) {
    if(!p) return;
    p->active_color=active; p->inactive_color=inactive;
}
