...
void bandwidthmon_init(BandwidthMon *b, uint64_t now_ms) {
    if(!b) return; memset(b,0,sizeof(*b)); b->window_start_ms=now_ms;
}
...
void bandwidthmon_record(BandwidthMon *b, int session_id, uint64_t in_bytes, uint64_t out_bytes) {
    if(!b) return; bm_session *s=bm_find(b,session_id); if(!s) return;
...
uint64_t bandwidthmon_total_in(const BandwidthMon *b) {
    if(!b) return 0; uint64_t t=0; for(int i=0;i<b->n;i++) t+=b->sessions[i].bytes_in; return t;
}
uint64_t bandwidthmon_rate_in(BandwidthMon *b, int session_id, uint64_t now_ms) {
    if(!b) return 0; bm_session *s=bm_find(b,session_id); if(!s) return 0;
...
void bandwidthmon_reset_window(BandwidthMon *b, uint64_t now_ms) {
    if(!b) return; for(int i=0;i<b->n;i++){ b->sessions[i].window_in=0; b->sessions[i].window_out=0; } b->window_start_ms=now_ms;
}
