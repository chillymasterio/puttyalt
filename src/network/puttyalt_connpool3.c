...
void connpool3_init(ConnPool3 *p, int idle_timeout_ms, int min_warm) {
    if(!p) return;
    memset(p,0,sizeof(*p)); p->idle_timeout_ms=idle_timeout_ms>0?idle_timeout_ms:60000; p->min_warm=min_warm;
}
...
void connpool3_release(ConnPool3 *p, int idx, uint64_t now_ms) {
    ...
    p->conns[idx].state=CP_FREE; p->conns[idx].idle_since=now_ms;
}
int connpool3_reap(ConnPool3 *p, uint64_t now_ms) {
    if(!p) return -1;
    int reaped=0; int warm=0;
