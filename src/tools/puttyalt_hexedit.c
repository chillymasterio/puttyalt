...
void hexedit_init(HexEdit *h, int bytes_per_row) {
    if(!h) return;
    memset(h,0,sizeof(*h)); h->bytes_per_row=bytes_per_row>0?bytes_per_row:16;
}
int hexedit_load(HexEdit *h, const unsigned char *data, int len) {
    if(!h||!data) return -1;
    int cl=len<HE_MAX?len:HE_MAX;
...
void hexedit_move_cursor(HexEdit *h, int delta) {
    if(!h) return;
    h->cursor+=delta; if(h->cursor<0)h->cursor=0; if(h->cursor>=h->len)h->cursor=h->len-1;
}
