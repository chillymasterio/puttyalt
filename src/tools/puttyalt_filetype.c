...
int filetype_by_magic(const unsigned char *data, int len) {
    ...
    if (data[0]==0xFF&&data[1]==0xD8) return FT_IMAGE; /* jpeg */
    if (data[0]=='P'&&data[1]=='K') return FT_ARCHIVE; /* zip */
    if (data[0]==0x1F&&data[1]==0x8B) return FT_ARCHIVE; /* gzip */
    if (data[0]==0x7F&&data[1]=='E'&&data[2]=='L'&&data[3]=='F') return FT_BINARY;
    if (data[0]=='M'&&data[1]=='Z') return FT_BINARY; /* PE */
