...
void cmdrunner_init(CmdRunner *c, const char *command) {
    if(!c) return;
    memset(c,0,sizeof(*c)); snprintf(c->command,256,"%s",command?command:"");
}
int cmdrunner_add_host(CmdRunner *c, const char *host) {
    if(!c||c->n>=CR_MAX||!host) return -1;
    snprintf(c->r[c->n].host,CR_HOST,"%s",host); c->r[c->n].exit_code=-1; return c->n++;
}
...
int cmdrunner_success_count(const CmdRunner *c) {
    if(!c) return -1;
    int n=0; for(int i=0;i<c->n;i++) if(c->r[i].done && c->r[i].exit_code==0)n++; return n;
}
