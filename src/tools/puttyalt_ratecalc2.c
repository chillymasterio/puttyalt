...
void ratecalc2_init(RateCalc2 *r, uint64_t target) {
    if(!r) return;
    for(int i=0;i<RC_SAMPLES;i++){r->samples[i]=0;r->times[i]=0;} r->head=0;r->count=0;r->total=0;r->target=target;
}
void ratecalc2_update(RateCalc2 *r, uint64_t cumulative, uint64_t now_ms) {
    if(!r) return;
    r->samples[r->head]=cumulative; r->times[r->head]=now_ms;
...
int64_t ratecalc2_eta_sec(const RateCalc2 *r) {
    if(!r||r->target==0) return -1;
    uint64_t bps=ratecalc2_bps(r); if(bps==0) return -1;
