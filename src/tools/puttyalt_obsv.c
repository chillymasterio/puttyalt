...
int obsv_incr(Observ *o, const char *name, int64_t by) {
    if(!o||!name) return -1;
    ob_metric *m=obsv_find(o,name,OB_COUNTER); if(!m) return -1; m->value+=by; return 0;
}
int obsv_set_gauge(Observ *o, const char *name, int64_t v) {
    if(!o||!name) return -1;
    ob_metric *m=obsv_find(o,name,OB_GAUGE); if(!m) return -1; m->value=v; return 0;
}
int obsv_observe(Observ *o, const char *name, int64_t val) {
    if(!o||!name) return -1;
    ob_metric *m=obsv_find(o,name,OB_HISTO); if(!m) return -1;
