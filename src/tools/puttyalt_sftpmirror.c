...
int sftpmirror_add_local(SftpMirror *m, const char *path, uint64_t mtime, uint64_t size) {
    if(!m||m->nl>=SM_MAX) return -1; sm_file *f=&m->local[m->nl++]; snprintf(f->path,SM_PATH,"%s",path); f->mtime=mtime; f->size=size; return 0;
}
int sftpmirror_add_remote(SftpMirror *m, const char *path, uint64_t mtime, uint64_t size) {
    if(!m||m->nr>=SM_MAX) return -1; sm_file *f=&m->remote[m->nr++]; snprintf(f->path,SM_PATH,"%s",path); f->mtime=mtime; f->size=size; return 0;
}
...
int sftpmirror_diff(SftpMirror *m, char *upload, int ulen, char *del, int dlen) {
    if(!m) return -1; int up=0, dl=0, changes=0;
