...
void sessquota_init(SessQuota *q, uint64_t max_bytes, int max_time_sec, int max_cmds) {
    if(!q) return;
    memset(q,0,sizeof(*q)); q->max_bytes=max_bytes; q->max_time_sec=max_time_sec; q->max_cmds=max_cmds;
}
...
int sessquota_bytes_pct(const SessQuota *q) {
    if(!q||q->max_bytes==0) return 0;
    return (int)((q->used_bytes*100)/q->max_bytes);
}
