...
int authmethod_mark_result(AuthMethod *a, int success) {
    if(!a||a->current<0) return -1; a->m[a->current].succeeded=success?1:0; return 0;
}
int authmethod_succeeded(const AuthMethod *a) {
    if(!a) return -1; for(int i=0;i<a->n;i++) if(a->m[i].succeeded) return a->m[i].method; return AM_NONE;
}
