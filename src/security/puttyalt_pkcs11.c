...
void pkcs11_init(Pkcs11 *p, const char *module_path) {
    if(!p) return; memset(p,0,sizeof(*p)); snprintf(p->module_path,160,"%s",module_path?module_path:"");
}
...
int pkcs11_find_token(const Pkcs11 *p) {
    if(!p) return -1; for(int i=0;i<p->n;i++) if(p->slots[i].has_token) return i; return -1;
}
void pkcs11_set_keys(Pkcs11 *p, int idx, int key_count, int login_required) {
    if(!p||idx<0||idx>=p->n) return; p->slots[idx].key_count=key_count; p->slots[idx].login_required=login_required?1:0;
}
int pkcs11_slot_count(const Pkcs11 *p) { return p?p->n:-1; }
int pkcs11_total_keys(const Pkcs11 *p) {
    if(!p) return -1; int n=0; for(int i=0;i<p->n;i++) n+=p->slots[i].key_count; return n;
}
