...
void fido2_init(Fido2 *f, const char *rp_id, int timeout_ms) {
    if(!f) return;
    memset(f,0,sizeof(*f)); snprintf(f->rp_id,96,"%s",rp_id?rp_id:"ssh:"); f->timeout_ms=timeout_ms>0?timeout_ms:30000;
}
int fido2_begin(Fido2 *f, int requires_pin, uint64_t now_ms) {
    if(!f) return -1;
    f->state=F2_WAITING_TOUCH; f->requires_pin=requires_pin?1:0; f->challenge_ms=now_ms; return 0;
}
