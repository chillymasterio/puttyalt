...
void chanwin_init(ChanWin *w, int32_t initial) {
    if(!w) return; w->local_win=w->remote_win=w->initial=initial>0?initial:131072;
    w->adjust_threshold=w->initial/2; w->consumed=0;
}
int chanwin_consume_local(ChanWin *w, int32_t n) {
    if(!w||n<0) return -1; if (n>w->local_win) return -1;
...
int32_t chanwin_adjust_local(ChanWin *w) {
    if(!w) return 0; int32_t add=w->initial - w->local_win; w->local_win=w->initial; return add;
}
int chanwin_consume_remote(ChanWin *w, int32_t n) {
    if(!w||n<0||n>w->remote_win) return -1; w->remote_win-=n; return 0;
}
